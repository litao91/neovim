  ...
  for (int i = 0; i < num_matches && !got_int; i++) {
    ...
    if (command_end != NULL) {
      ...
      while (*p && *p != '\r' && *p != '\n') {
        ...
        if (strncmp_(p, S_LEN("file:")) == 0 && ascii_isspace(p[5])) {
          p += 5;
          continue;
        }
...
static bool findtags_hdr_parse(findtags_state_T *st)
{
  ...
  if (strncmp_(st->lbuf, S_LEN("!_TAG_")) != 0) {
    // Non-header item before the header, e.g. "!" itself.
    return true;
  }

  // Process the header line.
  if (strncmp_(st->lbuf, S_LEN("!_TAG_FILE_SORTED\t")) == 0) {
    st->tag_file_sorted = (uint8_t)st->lbuf[18];
  }
  if (strncmp_(st->lbuf, S_LEN("!_TAG_FILE_ENCODING\t")) == 0) {
  ...
  if (find_extra(&p) == OK) {
    ...
    if (*p++ == TAB) {
      ...
      while (ASCII_ISALPHA(*p) || utfc_ptr2len(p) > 1) {
        if (strncmp_(p, S_LEN("kind:")) == 0) {
          tagp->tagkind = p + 5;
        } else if (strncmp_(p, S_LEN("user_data:")) == 0) {
          tagp->user_data = p + 10;
        } else if (strncmp_(p, S_LEN("line:")) == 0) {
...
static int find_extra(char **pp)
{
  ...
  if (str != NULL && strncmp_(str, S_LEN(";\"")) == 0) {
    *pp = str;
    return OK;
  }
  return FAIL;
...
int get_tags(list_T *list, char *pat, char *buf_fname)
{
  ...
  for (int i = 0; i < num_matches; i++) {
    ...
    if (strncmp_(tp.tagname, S_LEN("!_TAG_")) == 0) {
      xfree(matches[i]);
      continue;
    }
    ...
    if (tp.command_end != NULL) {
      for (char *p = tp.command_end + 3;
           *p != NUL && *p != '\n' && *p != '\r';
           MB_PTR_ADV(p)) {
        if (p == tp.tagkind
            || (p + 5 == tp.tagkind && strncmp_(p, S_LEN("kind:")) == 0)) {
          // skip "kind:<kind>" and "<kind>"
          p = tp.tagkind_end - 1;
